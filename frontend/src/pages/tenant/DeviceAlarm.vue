<template>
  <div>
    ...
    <ag-device-access-data-dialog :show.sync="dialog.detail" :device="dialog.item" :skip-access-status="3" :marker-access-status="2" show-access-codes></ag-device-access-data-dialog>
  </div>
</template>
<script>
  ...
  methods: {
    filter () {
      return this.initedPromise.then(() => {
        let result = { 'accessState.status': 2, farmland: Filter.farmland(this), type: Filter.deviceType(this) }
