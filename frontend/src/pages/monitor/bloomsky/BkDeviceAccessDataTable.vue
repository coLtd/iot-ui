...
export default {
  name: 'bk-device-access-data-table',
  props: {
    device: {
      type: Object,
      required: true
      ...
      ps.then(items => {
        ...
        items.items.forEach(item => {
          _this.table.items.push({
            ...
            temperature: item.data['61'].v,
            humidity: item.data['62'].v,
            atmospheric: item.data['67'].v,
            wspeed: item.data['63'].v,
            wdirection: item.data['64'].v === 9999 ? '-' : d2t.direction2Text(item.data['64'].v),
